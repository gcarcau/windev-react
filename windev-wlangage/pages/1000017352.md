...
**Remarque** : Cette fonction est identique à la fonction [TableauInsèreLigne](3075014.md).
...
```wlangage
InsèreLigne(<Tableau WLangage> , <Indice d'insertion> [, <Élément colonne 1> [... [, <Élément colonne N>]]])
```
...
> Si le paramètre <Indice d'insertion> est égal au nombre de lignes du tableau +1, la ligne est ajoutée à la fin du tableau (équivalent à la fonction [TableauAjouteLigne](3075013.md)).
> Si le paramètre <Indice d'insertion> est supérieur au nombre de lignes du tableau + 1, une erreur WLangage est générée.
...
Lors de l'utilisation de la fonction **InsèreLigne** :
...
Lors de la déclaration d'un tableau de N par M éléments, ce tableau contient N lignes vides. Par exemple, le tableau déclaré ci-dessous contient 3 chaînes vides.
...
Lors de l'insertion d'une ligne (fonction **InsèreLigne**), cette ligne est automatiquement insérée parmi les lignes déjà présentes dans le tableau.
...
- Cette fonction ne peut pas être utilisée sur :
  ...
  - les tableaux fixes.
- Pour ajouter une ligne à la fin d'un tableau à deux dimensions, utilisez la fonction [TableauAjouteLigne](3075013.md) ou la fonction [AjouteLigne](1000017349.md).
