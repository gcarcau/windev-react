---
title: "TableAjouteColonne (Fonction)"
english: "TableAddColumn"
type: "Fonction"
id: "1000017168"
module: "WDLang1"
...
- Si le champ Table est trié avec la fonction [TableTrie](3074035.md), la colonne ajoutée est positionnée dans le champ en fonction du tri en cours.
...
Si la saisie en cascade n'est pas activée, le fichier de données lié au champ Table fichier n'est pas modifié. Pour écrire le contenu de la colonne ajoutée dans le fichier de données lié, il est nécessaire d'utiliser la fonction [TableEnregistre](3074008.md) après la fonction **TableAjouteColonne**.
...
A chaque appel de la fonction **TableAjouteColonne** dans un champ Table fichier, une colonne virtuelle est créée dans le champ. Les informations seront enregistrées dans le fichier de données uniquement lors de l'appel de la fonction [TableEnregistre](3074008.md).
Si la fonction **TableAjouteColonne** est appelée successivement dans le même traitement sans être suivie de la fonction [TableEnregistre](3074008.md), seul le PREMIER appel est effectif (les appels suivants sont ignorés). Une seule colonne est ajoutée dans le champ Table fichier.

### Ajout dans un champ Table fichier et Trigger
Il est possible de déclencher un trigger lors de l'ajout d'un enregistrement dans un champ Table fichier. Pour plus de détails, consultez la documentation de [HDécritTrigger](3044074.md).
...
- [<Table>.AjouteColonne (Fonction)](1000023607.md)
