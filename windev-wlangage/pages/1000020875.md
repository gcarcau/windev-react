...
> - Faux si le document n'a pu être ni enregistré ni visualisé. Le message d'erreur correspondant peut être connu avec la fonction [ErreurInfo](3013008.md).
...
> - Vrai si l'interface utilisateur du périphérique Twain utilisé doit être affichée. Lors de l'enregistrement du document, le <Fichier TIFF> sera proposé par défaut.
...
- soit au document présent dans un scanneur au moment de l'appel de la fonction **TwainVersTIFF**. Dans ce cas, les paramètres spécifiés grâce à la fonction [TwainZoneScan](3078014.md) sont pris en compte.
