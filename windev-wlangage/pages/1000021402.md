---
title: "SourceDepuisFenêtre (Fonction)"
english: "SourceFromWindow"
type: "Fonction"
id: "1000021402"
module: "WDLang4"
...
```wlangage
SourceDepuisFenêtre(<Source> [, <Fenêtre>])
```
...
- Pour affecter automatiquement la valeur NULL à une rubrique avec la fonction **SourceDepuisFenêtre** :
...
- Les rubriques du fichier de données non reliées à un champ ne sont pas mises à jour par la fonction **SourceDepuisFenêtre**.
- Si plusieurs champs de la fenêtre (ou de la page) sont reliés à une même rubrique du fichier de données, la rubrique reliée peut ne pas être mise à jour correctement par la fonction **SourceDepuisFenêtre**. ***Il est déconseillé de relier plusieurs champs d'une fenêtre (ou d'une page) à une même rubrique d'un fichier de données***.
- La fonction **SourceDepuisFenêtre** n'affecte pas les rubriques pour les champs de type Image. Utilisez la fonction [HAttacheMémo](3044173.md).
- La fonction **SourceDepuisFenêtre** ne fonctionne pas avec les vues.
...
- [<Fenêtre>.VersSource (Fonction)](1000021386.md)
- [<Source>.DepuisFenêtre (Fonction)](1000021403.md)
