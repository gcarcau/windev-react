...
SI PAS HExécuteRequête(UneRequête) ALORS
	Trace(HErreurInfo())
SINON
	SI PAS HExporteXML(UneRequête, "FicTest.xml", hExpCréation) ALORS
		Trace(HErreurInfo())
	FIN
FIN
```
...
Pour exporter le contenu d'une requête, il est nécessaire d'initialiser la requête (fonction [HExécuteRequête](3044080.md)) avant d'utiliser la fonction **HExporteXML**.

### Exporter un seul enregistrement
Pour exporter un seul enregistrement vers une chaîne XML, utilisez la fonction [HEnregistrementVersXML](3044093.md).
