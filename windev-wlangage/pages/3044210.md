---
title: "FichierVersEcran (Fonction)"
english: "FileToScreen"
type: "Fonction"
id: "3044210"
module: "WDLang4"
...
- La fonction **FichierVersEcran** n'effectue aucune lecture du fichier de données. C'est la valeur de l'enregistrement chargé en mémoire qui est utilisée pour initialiser les champs. Il faut donc qu'un enregistrement soit au préalable chargé en mémoire (fonctions [HLit](3044047.md), [HLitRecherche](3044050.md), [HLitPremier](3044051.md), ...).
...
- En WEBDEV, vous pouvez également utiliser la fonction [FichierVersPage](3044148.md).
...
```wlangage
FichierVersEcran([<Fenêtre/Page> [, <Source>]])
```

`<Fenêtre/Page>` *Nom de fenêtre ou nom de page*
...
- **Champ Combo auto-alimenté lié à un fichier de données HFSQL** : La fonction **FichierVersEcran** ne doit pas se trouver dans l'événement "Déclarations globales" de la fenêtre. La fonction doit être présente dans l'événement "Initialisation" de la fenêtre.
- Les espaces dans les rubriques HFSQL sont conservés.
- La fonction **FichierVersEcran** initialise les champs Image reliés à des rubriques de type mémo image.
- La fonction **FichierVersEcran** gère la valeur NULL. Si le contenu de la rubrique correspond à NULL, une chaîne vide sera affichée dans le champ de saisie associé. Pour modifier ce mode d'affichage, utilisez la fonction [ChampErreur](3025036.md).
- La fonction **FichierVersEcran** ne fonctionne pas avec les vues. Il est nécessaire d'initialiser un à un les champs avec le contenu des rubriques.
