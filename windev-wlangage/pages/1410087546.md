...
- L'onglet "Synthèse" présente les dix traitements les plus consommateurs de temps. Dans notre exemple, nous pouvons voir que la procédure locale "MetAJourStockProduits" prend plus de 3 secondes à s'exécuter.
  [image: Analyseur de performances - Synthèse]
- L'onglet "Cartographie" permet d'identifier de manière visuelle ce qui a pris le plus de temps. Dans notre cas, c'est un appel à la fonction [Ping](3014013.md) :
  [image: Analyseur de performances - Cartographie]
- L'onglet "Détail" présente tous les traitements ou événements appelés du plus long au plus rapide.
  [image: Analyseur de performances - Détail]
...
5. La ligne de code exécutée est la suivante :
  ...
  ```
  Le ralentissement vient du fait que l'adresse spécifiée pour la fonction [Ping](3014013.md) est inaccessible.
...
1. Remplacez la ligne de code contenant la fonction [Ping](3014013.md) par la ligne de code suivante :
  ```wlangage
  // Vérifie l'accessibilité du serveur fournisseur
  Ping("www.google.fr")
  ```
...
6. Sous l'onglet "Cartographie", la fonction [Ping](3014013.md) n'apparaît plus avec la même importance.
  [image: Analyseur de performances - Cartographie]
...
Dans ce cas, seul le code présent entre les fonctions [AnalyseurDébut](3013012.md) et [AnalyseurFin](3013002.md) est analysé. Le résultat est enregistré sous forme d'un fichier WPF qui peut être lu par WINDEV.
Pour plus de détails, consultez [Analyseur de performances](2030035.md).

| [Leçon précédente](1410087545.md) | [Sommaire](1410087560.md) |
| --- | --- |
