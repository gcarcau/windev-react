---
title: "<Page>.DepuisSource (Fonction)"
english: "<Page>.FromSource"
type: "Fonction"
id: "1000021393"
module: "WDLang4"
...
- La fonction **<Page>.DepuisSource** n'effectue aucune lecture du fichier de données. C'est la valeur de l'enregistrement chargé en mémoire qui est utilisée pour initialiser les champs. Il faut donc qu'un enregistrement soit au préalable chargé en mémoire (fonctions [<Source>.Lit](1000025019.md), [<Source>.LitRecherche](1000025023.md), [<Source>.LitPremier](1000025022.md), ...).
...
SI HTrouve() = Vrai ALORS PAGE_Client.DepuisSource()

// Lecture de l'enregistrement et
// transfert des rubriques vers les champs
HLitPremier(CLIENT, NOM)
PAGE_Client.DepuisSource()
```

## Syntaxe

```wlangage
<Page>.DepuisSource([<Source>])
```
...
- *liaison directe* : Dans ce cas, la fonction **<Page>.DepuisSource** transfère la valeur des rubriques de l'enregistrement chargé en mémoire dans le champ relié, sans effectuer de lecture.
- *liaison par recherche dans un fichier de données relié*.

### Divers

- Les espaces dans les rubriques HFSQL sont conservés.
- La fonction **<Page>.DepuisSource** affecte les champs Image reliés à des rubriques de type mémo image.
- La fonction **<Page>.DepuisSource** ne fonctionne pas avec les vues. Il est nécessaire d'affecter une à une les rubriques aux champs.
...
- [SourceVersPage (Fonction)](1000017405.md)
